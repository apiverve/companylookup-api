...
class CompanylookupAPIClientError(Exception):
...
class ValidationError(CompanylookupAPIClientError):
...
class CompanylookupAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"ticker": {"type": "string", "required": True, "minLength": 1, "maxLength": 5}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the CompanylookupAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises CompanylookupAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/companylookup'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise CompanylookupAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[CompanylookupAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns CompanyLookupData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises CompanylookupAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise CompanylookupAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise CompanylookupAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise CompanylookupAPIClientError(f"Request failed: {str(e)}")
