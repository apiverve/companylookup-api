...
class CompanylookupValidationException implements Exception {
  final List<String> errors;

  CompanylookupValidationException(this.errors);

  @override
  String toString() => 'CompanylookupValidationException: ${errors.join("; ")}';
}
...
/// Company Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/companylookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ticker] (required) - Stock ticker symbol (e.g. AAPL, MSFT, GOOGL) [minLength: 1, maxLength: 5]
class CompanylookupClient {
  final String apiKey;
  ...
  CompanylookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/companylookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CompanylookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CompanylookupValidationException(errors);
    }
  }

  /// Execute a request to the Company Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<CompanylookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CompanylookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CompanylookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CompanylookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CompanylookupException('Resource not found');
      } else {
        throw CompanylookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CompanylookupException) rethrow;
      throw CompanylookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Company Lookup API client.
class CompanylookupException implements Exception {
  final String message;

  CompanylookupException(this.message);

  @override
  String toString() => 'CompanylookupException: $message';
}
